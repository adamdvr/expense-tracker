...
src/
  ...
  widgets/      # Композитные блоки UI (напр. auth-layout — обёртка для страниц логина/регистрации, app-shell — сайдбар авторизованных страниц, categories-list — страница категорий)
  features/     # Пользовательские сценарии с логикой (напр. auth/login, auth/register, transaction/create, category/create)
...
- **Запросы к API**: функции запросов, фабрика ключей кэша (`transactionKeys`, `categoryKeys`) и query-хуки (`useTransactions`, `useCategories`) лежат в `entities/<сущность>/api`; мутации пользовательских сценариев (`useCreateTransaction`, `useCreateCategory`) — в `features/<домен>/<сценарий>/api` и после успеха инвалидируют кэш по корневому ключу сущности.
...
- **Форма отделена от диалога**: `CreateTransactionForm` не завязана на `Dialog` — принимает `renderFooter` пропом, что позволяет переиспользовать форму вне модалки (напр. на отдельной странице). Так же устроена `CreateCategoryForm`.

### Категории: иконка и цвет

- **Иконка**: backend хранит в `icon` имя lucide-иконки в kebab-case (`shopping-cart`). Компонент берётся из статического реестра `CATEGORY_ICONS` (`entities/category/config/icons.ts`), имя не из реестра рисуется иконкой `Tag`. `DynamicIcon` из `lucide-react/dynamic` не используем — он грузит каждую иконку отдельным чанком и тянет в бандл карту имён всех иконок. Новая иконка добавляется в реестр.
- **Цвет** новой категории — только из палитры `CATEGORY_COLORS` (`entities/category/config/colors.ts`). Для каждого цвета проверен контраст ≥ 3:1 (WCAG 1.4.11) и кружка на светлом фоне, и иконки на подложке `CategoryIcon` (цвет, разбавленный до 16% через `color-mix`). Новый цвет добавлять только после такой проверки. Цвет категории — пользовательские данные, поэтому задаётся inline-стилем, а не токеном темы.
- **Выбор цвета и иконки** в `CreateCategoryForm` — нативные `<input type="radio">`: визуально скрыты внутри `<label>`, подключены через `register()`, выбор и фокус рисует соседний элемент (`peer-checked:` / `peer-focus-visible:`). Стрелки, Tab и скринридеры работают без своего кода.
